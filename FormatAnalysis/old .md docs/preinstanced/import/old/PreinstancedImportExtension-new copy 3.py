    ...
    def execute(self, context: bpy.types.Context) -> set:
        ...
        for x in mshBytes.finditer(tmpRead):
            ...
            for i in range(mDataSubCount):
                try:
                    # Hardcoded UV decode settings: AUTO_TAIL off8 invV1 (no U invert)
                    uv_method = 'AUTO_TAIL'
                    uv_tail_bytes = 8
                    invert_v = True
                    bPrinter(f"[UV] Using hardcoded decode: {uv_method} @ tail {uv_tail_bytes}, invert_v={invert_v}")
...
class MyAddonPreferences(bpy.types.AddonPreferences):
    ...
    def draw(self, context: bpy.types.Context) -> None:
        ...
        layout.label(text="UV decoding is hard-coded: AUTO_TAIL @ 8 bytes, Invert V = ON")
