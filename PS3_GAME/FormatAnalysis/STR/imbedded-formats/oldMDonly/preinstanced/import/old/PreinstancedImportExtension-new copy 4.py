            ...
            for i in range(mDataSubCount):
                ...
                try:
                    ...
                    for v in range(VertCount):
                        ...
                        # Use fixed offsets discovered by analyzer: main UV at 0x14, CM UVs at 0x1C
                        FIXED_UV_OFFSET = 0x14  # 20 bytes from start of vertex
                        FIXED_CM_OFFSET = 0x1C  # 28 bytes from start of vertex

                        # Main UVs
                        uv_offset = vert_data_start + FIXED_UV_OFFSET
                        if uv_offset + 8 <= len(tmpRead):
                            data_io.seek(uv_offset)
                            TempUV = struct.unpack('>ff', data_io.read(8))
                        else:
                            bPrinter(f"[MeshPart {mesh_iter}_{i}] Warning: Insufficient data for UV coords at {uv_offset:08X} for vertex {v}. Skipping UV.")
                            TempUV = (0.0, 0.0)
                        # Flip V per reference
                        UVTable.append((TempUV[0], 1 - TempUV[1]))

                        # Secondary (CM) UVs
                        cm_offset = vert_data_start + FIXED_CM_OFFSET
                        if cm_offset + 8 <= len(tmpRead):
                            data_io.seek(cm_offset)
                            TempCM = struct.unpack('>ff', data_io.read(8))
                        else:
